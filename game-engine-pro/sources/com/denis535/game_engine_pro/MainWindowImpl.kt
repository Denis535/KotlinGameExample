...
public abstract class MainWindowImpl : MainWindow {
    public companion object {
        ...
        @OptIn(ExperimentalForeignApi::class)
        private fun MouseButton.ToInt(): Int {
            return when (this) {
    ...
    public override fun Show() {
        check(!this.IsClosed)
        glfwSetCursorEnterCallback(this.NativeWindow, MainWindowImpl.OnMouseCursorEnterCallback).also { GLFW.ThrowErrorIfNeeded() }
        glfwSetCursorPosCallback(this.NativeWindow, MainWindowImpl.OnMouseCursorMoveCallback).also { GLFW.ThrowErrorIfNeeded() }
        glfwSetMouseButtonCallback(this.NativeWindow, MainWindowImpl.OnMouseButtonActionCallback).also { GLFW.ThrowErrorIfNeeded() }
        glfwSetScrollCallback(this.NativeWindow, MainWindowImpl.OnMouseWheelScrollCallback).also { GLFW.ThrowErrorIfNeeded() }
        glfwSetKeyCallback(this.NativeWindow, MainWindowImpl.OnKeyActionCallback).also { GLFW.ThrowErrorIfNeeded() }
        glfwSetCharCallback(this.NativeWindow, MainWindowImpl.OnCharActionCallback).also { GLFW.ThrowErrorIfNeeded() }
    ...
    public override fun GetMouseButtonPressed(button: MouseButton): Boolean {
        check(!this.IsClosed)
        return glfwGetMouseButton(this@MainWindowImpl.NativeWindow, button.ToInt()).also { GLFW.ThrowErrorIfNeeded() } == GLFW_PRESS
    }

    @OptIn(ExperimentalForeignApi::class)
    public override fun GetKeyPressed(key: Key): Boolean {
        check(!this.IsClosed)
        return glfwGetKey(this@MainWindowImpl.NativeWindow, key.ToInt()).also { GLFW.ThrowErrorIfNeeded() } == GLFW_PRESS
    }
